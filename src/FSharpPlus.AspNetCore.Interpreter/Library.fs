module FSharpPlus.AspNetCore.Interpreter
open FSharpPlus
...
type WebPart<'a> = 'a -> OptionT<Async<'a option>>
type Pattern = { pattern: string; httpMethods: string list;  }
type Endpoint<'a> = Pattern -> Async<'a option>
type Router<'a> = List of Endpoint<'a>
//
...
module Filters=
  ...
  let path (s:string) = let path =  s in OptionT << fun (x : Http.Context) -> async.Return (if (path = x.request.Path) then Some x else None)
...
module I=
  let ofRequest (httpRequest:HttpRequest) = failwith ""
  let ofHttpContext (httpContext:HttpContext)=
      { request = ofRequest httpContext.Request ; response = Response.empty }
...
let appRun (app:WebPart<Context>) (appBuilder:IApplicationBuilder)=
  ...
  let runApp context = task {
    let ctx = I.ofHttpContext context
    let! task = app ctx |> OptionT.run |> Async.StartAsTask
    match task with
    | Some res-> return! I.yieldToResponse res.response context.Response
